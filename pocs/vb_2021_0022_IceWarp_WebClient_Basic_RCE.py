...
class IceWarpWebClientBasicRCE(BasePoc):
    ...
    def verify(self, first=False):
        ...
        if resp is not None:
            location = resp.text
            # print (location)
            if resp.status_code == 302 and 'IP Address' in location:
                ...
                if verbose:
                    highlight('[*] IceWarp WebClient Basic Found')
            else:
                if verbose:
                    highlight('[*] IceWarp WebClient Basic Not Found')                        
