...
POC_NAME = "ICEFlowVPNInformationLeakage"    # PoC脚本中实现的类名，TCC框架将根据POC_NAME去实例化类以达到调用的效果，因此类名应与该变量名保持相同


class ICEFlowVPNInformationLeakage(BasePoc):
    ...
    poc_info = {
        'poc': {
            'Id': 'vb_2021_0021',    # PoC的VID编号
            'vbid': '',
            'Name': 'ICEFlow VPN Information Leakage',    # PoC名称
            'Author': 'magic_rookie',    # PoC作者
            'Create_date': '2021-7-17',    # PoC创建时间
            },

        'vul': {
            'Product': 'ICEFlow VPN ',    # 漏洞所在产品名称
            'Version': 'ICEFlow VPN',    # 产品的版本号
    ...
    def verify(self, first=False):
        ...
        if resp is not None:
            ...
            if 'session' or 'ICEFLOW 'in location:
                ...
                if verbose:
                    highlight('[*] ICEFlow VPN Information Leakage Found')
            else:
                if verbose:
                    highlight('[*] ICEFlow VPN Information Leakage Not Found')                        
